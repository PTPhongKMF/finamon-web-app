    ...
    <HoverCard openDelay={0} closeDelay={50}>
      ...
      <HoverCardContent>
        <nav className="flex flex-col rounded-md gap-2 p-2 text-sm text-gray-700 bg-[url(/images/navbar_bg.svg)] bg-cover">
          <Link to="/profile" className="block p-2 border-b-2 border-transparent hover:border-b-2 hover:border-green-300 w-full">Tài Khoản</Link>
          {user.roles.some(role => role === "Staff") && (
          <Link to="/dashboard/staff" className="block p-2 border-b-2 border-transparent hover:border-b-2 hover:border-green-300 w-full">Staff Dashboard</Link>
          )}
          {user.roles.some(role => role === "Admin") && (
          <Link to="/dashboard/admin" className="block p-2 border-b-2 border-transparent hover:border-b-2 hover:border-green-300 w-full">Admin Dashboard</Link>
          )}
